...
supermodel <- read.table("http://people.fas.harvard.edu/~mair/datasets/Supermodel.dat", header = TRUE)
View(supermodel)
#* 4 columns: SALARY, AGE, YEARS, BEAUTY
...
#           (If you do this bonus, reset the plot window once you're done.)


hist(supermodel$AGE, breaks=20)
hist(supermodel$SALARY, breaks=50)
range(supermodel$AGE)
range(supermodel$SALARY)
#* Bonus: 
op <- par(mfrow=c(1,2))
hist(supermodel$AGE, breaks=20)
hist(supermodel$SALARY, breaks=50)
...
plot(supermodel$AGE, supermodel$SALARY, 
     xlim=c(10,25), ylim=c(0,100),
     xlab="Model Age", ylab="Salary", main="Age vs Salary for supermodels")
#* Age of highest paid: looks like 24 years old
#* We can find out!
  supermodel[supermodel$SALARY == max(supermodel$SALARY), "AGE"] #Answer: 24.2 years old
#* Bonus:
avg.beauty <- mean(supermodel$BEAUTY)
plot(supermodel$AGE, supermodel$SALARY, 
     xlim=c(10,25), ylim=c(0,100),
     xlab="Model Age", ylab="Salary", main="Age vs Salary for supermodels", 
     col=ifelse(supermodel$BEAUTY < avg.beauty, "black", "magenta"),
     pch=ifelse(supermodel$BEAUTY < avg.beauty, 4, 2))
# The added arguments color each datapoint based on whether that model was of above average beauty
# What we see is that it's not so much beauty, but rather age, that seems to be largely driving salary.
...
qqnorm(supermodel$AGE)
qqnorm(supermodel$SALARY)
shapiro.test(supermodel$SALARY)        
shapiro.test(supermodel$AGE) 
ks.test(supermodel$AGE, "pnorm", mean = mean(supermodel$AGE), sd = sd(supermodel$AGE))  
ks.test(supermodel$SALARY, "pnorm", mean = mean(supermodel$SALARY), sd = sd(supermodel$SALARY))

cor.test(supermodel$AGE, supermodel$SALARY, method="pearson")
cor.test(supermodel$AGE, supermodel$SALARY, method="spearman")
...
#
#         - Regression coefficient estimate for AGE # 2.6301
#         - 95% confidence interval for AGE regression coefficient # 1.839466   3.420736
#         - Intercept value (What does this value represent, practically?) # -36.1820 (the salary of someone 0 years old)
#         - The t-value and p-value for AGE coefficient # t: 6.555 p: 3.65e-10
#         - R-squared value (adjusted and non-adjusted) for the regression model # 0.158, 0.1543
#
...
#
#                 Why do the values in each pair of lines come out equal to one another? 

#                 ANS: The default output of cor() is Pearson's r.  r.squared is just this value squared.

#       8.  What percent of the variance in SALARY is explained by AGE? 
#           What statistic tells you this information? 

#           ANS: Using adjusted r-squared, 15.43% of variance in salary is explained by age.


#       9.  Does age have a significant impact on salary?
#           Can you reject or fail to reject the null hypothesis, based on this finding?

#           ANS: Yes, age has a significant impact on salary. The older, the better paid. 
#                 If null hyp. was that age has no impact on salary, we can reject the null.



## This is a day's work well done.  Next time we'll continue exploring the world of high fashion, with multiple regression!
